...
import org.geektimes.base.bean.ComponentInfo;
import org.geektimes.base.function.ThrowableAction;
 ...
 * 组件上下文（Web 应用全局使用）
 */
public class ComponentContext implements ComponentSearch{
    ...
    private  Map<String, ComponentInfo> beanInfoMap = new HashMap<>();
    private  Map<Class<? extends Annotation>, List<ComponentInfo>> annotationComponentInfoMap = new HashMap<>();
    private  Map<Class, List<ComponentInfo>> classComponentInfoMap  = new HashMap<>();
...
//    }
    ...
    private Map<String, ComponentInfo> componentsMap = new LinkedHashMap<>();
...
//        init();
//    }
    public void init(ClassLoader classLoader) throws RuntimeException {
    ...
    protected void instantiateComponents() {
        ...
        componentNames.forEach(name -> {
            Object o = lookupComponent(name);
            ComponentInfo componentInfo = new ComponentInfo();
            componentInfo.setName(name);
            componentInfo.setInstance(o);
            Annotation[] as = o.getClass().getAnnotations();
            componentInfo.setAnnotations(as);
            Class c = o.getClass();
            componentInfo.setInstanceClass(c);
            componentsMap.put(name, componentInfo);
            for (Annotation a: as) {
                annotationComponentInfoMap.computeIfAbsent(a.annotationType(), (x) -> new ArrayList<>()).add(componentInfo);
            }
            classComponentInfoMap.computeIfAbsent(c, (x) -> new ArrayList<>()).add(componentInfo);
        });
    }
    ...
    protected void initializeComponents() {
        componentsMap.values().forEach(component -> {
            Class<?> componentClass = component.getInstanceClass();
            // 注入阶段 - {@link Resource}
            injectComponents(component.getInstance(), componentClass);
            // 初始阶段 - {@link PostConstruct}
            processPostConstruct(component.getInstance(), componentClass);
        });
    }
    ...
    public void destroy() throws RuntimeException {
        componentsMap.values().forEach(component -> {
            Class<?> componentClass = component.getInstanceClass();
            processPreDestroy(component.getInstance(), componentClass);
        });
    ...
    public <T> List<T> getComponentByAnnotation(Class<? extends Annotation> annotation) {
        List<ComponentInfo> l = annotationComponentInfoMap.get(annotation);
        List result = new ArrayList();
        for (ComponentInfo c : l) {
            result.add(c.getInstance());
        }
    ...
    public <T> List<T> getComponentByClass(Class aClass) {
        List<ComponentInfo> l = classComponentInfoMap.get(aClass);
        List result = new ArrayList();
        for (ComponentInfo c : l) {
            result.add(c.getInstance());
        }
    ...
    public <T> T getComponentByName(String name) {
        return (T) componentsMap.getOrDefault(name, new ComponentInfo()).getInstance();
    }
}
